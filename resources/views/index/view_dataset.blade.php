@extends('index.index')
@section('title', $view_datasets->dataset_title)
@section('web-content')
<section id="view_dataset"></section>
<div class="row mx-0 my-lg-5 py-5 p-lg-2 p-lg-0 p-4 mb-3">
  <div class="col-lg-12 px-lg-2 px-0">
    ...
    <h2 class="view-pub-title m-0">{{$view_datasets->dataset_title}}</h2>
    <div class="view-pub-details font-netflix py-1">
      <small>
        <span class="pr-1"><i class="fa fa-institution pr-1"></i> {{$view_datasets->dataset_contributor}}</span>
        <span class="pr-1"><i class="fa fa-calendar-o pr-1"></i> {{date('F Y', strtotime($view_datasets->dataset_date))}}</span>
  ...
  <div class="col-lg-8 px-lg-2 px-0 text-justify">
    <p class="view-pub-description px-lg-2 px-0 pt-3 border-top">{{ $view_datasets->dataset_description }}</p>
    <div class="row m-0 border-bottom px-lg-2 px-0 pb-4">
      <div class="col-lg-12 p-0">
        <div class="citation-div font-netflix-light">
          You can <b>cite</b> this dataset by clicking this
          <button class="btn-secondary shadow px-3 py-1 mx-1 rounded" data-target="#citationModal" data-toggle="modal" id="btnCitation" data-id="{{$view_datasets->dataset_id}}"> cite <i class="fa fa-quote-right"></i> </button> button.
        </div>
      </div>
    </div>
    <div class="row m-0 py-3">
      <div class="col-lg-12 px-lg-2 py-1 px-0">
        <div>
          ...
          <div>
            <span class="font-netflix-light" id="needAuth" >{{$view_datasets->dataset_author}}</span>
          </div>
        </div>
      </div>
      <div class="col-lg-4 px-lg-2 py-1 px-0">
        ...
        <div>
          <span class="font-netflix-light">{{$view_datasets->dataset_contributor}}</span>
        </div>
      </div>
      <div class="col-lg-4 px-lg-2 py-1 px-0 text-lg-center">
        ...
        <div>
          <span class="font-netflix-md">{{$downloadDatasets->where('download_dataset_id','=', $view_datasets->dataset_id)->count()}}</span> times
        </div>
      </div>
    </div>
    <div class="row m-0 border-bottom p-0">
      ...
      <div class="col-lg-12 d-flex justify-content-center px-2 pb-4">
        <button type="button" class="download-btn py-2 px-3 rounded font-netflix-light " data-user="{{Auth::user()->id}}" data-publication="{{$view_datasets->dataset_id}}" data-toggle="modal" data-target="#downloadModal">
  ...
  <div class="col-lg-4 my-lg-0 my-3 px-lg-3 p-0">
    <div class="related-content mb-3 px-1">
      ...
      <div class="suggested-container p-2 bg-light">
        ...
        @else
        @foreach ($relatedDatasets as $dataset)
        <div class="item px-3 py-2 bg-white m-1 dark-link border-bottom">
          <a href="{{ url('/dataset_id='.$dataset->dataset_id) }}" id="incrementButton" data-id="{{$dataset->dataset_id}}">
            <div>
              <small class="font-netflix-light font-weight-bold">{{$dataset->dataset_title}}</small>
            </div>
          </a>
          <div class="d-flex pt-1">
            <span>
              <small class="text-muted font-netflix-light pr-3"> {{ date('Y', strtotime($dataset->dataset_date)) }} </small>
            </span>
            <span>
              <small class="text-muted font-netflix-light related-authors">{{ $dataset->dataset_author }} </small>
            </span>
          </div>
        </div> 
        @endforeach
        <div class="font-netflix text-center p-1">
          <small><a href="{{ url('/datasets') }}">Check out more!</a></small>
        </div>
        @endif
      </div>
    </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body bg-light rounded m-3 generated-citation" id="citation">
          {{$view_datasets->generateCitation($view_datasets)}}
        </div>
        ...
        <div class="modal-content">
            <div class="modal-title pt-3 pb-3 px-4">
                ...
                <small>Kindly inform us of your purpose for downloading this publication and provide additional information to enhance our services, rest assured that your responses will be kept strictly confidential and not shared with 
                    any external parties. The survey will only take two minutes of your time. Thank you.</small>
            </div>
            <div class="modal-body bg-light rounded m-3">
                <form action="" method="" enctype="multipart/form-data" id="downloadForm">
                    @csrf
                    <div class="form-row">
                        <div class="col-md-12 form-group">
                          <small>Reason*</small>
                          <!-- <input type="text" class="form-control" name="download_reason" id="download_reason" value="" required> -->
                        ...
                        <div class="col-lg-12 p-3 d-flex justify-content-center">
                            <div class="row">
                                <div class="col">
                                    ...
                                    <button id="btnDownload" type="submit" class="download-now-btn px-4 py-2 rounded" data-id="{{$view_datasets->dataset_id}}">Download</button>
                                </div>
                                <div class="col">
...
$(document).ready(function () {
    ...
    $('body').on('click','.copy-citation-btn',function (e) {
    e.preventDefault();
    Swal.fire({
      title: 'Great!',
    ...
    $('#downloadForm').submit(function (e) { 
        e.preventDefault();
        var dataset_id = $('#btnDownload').data('id');
        ...
        $.ajax({
            type: "POST",
            url: "{{ url('download/dataset_id=')}} "+dataset_id,
            data: {
